...
class ArgumentObject{
  ...
  public:
    ...
    std::string getPropertyValues(std::string property,std::string option){
      ...
      std::string err = "Unable to recognize property or option "+property+" "+option;
    ...
    void setArgPropertyOpt(std::string property,std::string option, std::vector<std::string> val){
      bool setval = false;
      for(auto prop : string_vec_propobjs_){
        if(property.compare(prop->getPropertyName())==0){
          prop->setPropOption(option,val);
