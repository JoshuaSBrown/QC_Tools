...
void PropertyStringChoice::setPropOption(std::string option,std::string var){
  ...
  if(option.compare("STRING_CHOICE_ENFORCED")==0){
    if(var.compare("false")==0 || var.compare("true")==0){
      ...
      throw invalid_argument("The option "+option+" can only be set to values"
        " of true or false you have set it to "+var);
    }
  }else if(option.compare("STRING_CHOICES")==0){
    set<string> set_var{var};
    setPropOption_(option,set_var);
  }
  throw invalid_argument("Unrecognized option value combo "+option+" "+var);
}

void PropertyStringChoice::setPropOption(std::string option,set<string> set_vars){
  ...
  if(option.compare("STRING_CHOICES")==0){
    ...
    if(set_vars.size()>1){
      ...
      trim(err);
      throw invalid_argument("Option STRING_CHOICE_ENFORCED only accepts one "
        "value either true or false you have passed the following values "+err);
    }
...
bool PropertyStringChoice::propValid(const string & string_choice){
  ...
  if(val.compare("true")==0){
    ...
    throw invalid_argument("The value "+string_choice+" is not a valid option");
  }
  return true; 
}
