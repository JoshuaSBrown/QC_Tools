...
void PropertySisterFile::setPropOption(std::string option,std::string var){
  ...
  if(option.compare("SISTER_FILE_EXISTS")==0 ||
    ...
    throw invalid_argument("This option is determined internally you do not have permission to change it");
  }
  throw invalid_argument("Unrecognized option value combo "+option+" "+var);
}

void PropertySisterFile::setPropOption(std::string option,vector<string> vec_vars){
